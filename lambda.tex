...
%TCIDATA{LastRevised=Tuesday, April 26, 2016 20:15:15}
...
except for the Todd homomorphism section, is also in Knutson's book \cite{Knut73},
albeit sometimes with a different proof. Part of the plan was to add some
results from the Fulton/Lang book \cite{FulLan85} with better proofs, but this is not currently my
short-term objective, given that I don't understand much of \cite{FulLan85} to begin with.
Most of the notes was written independently of Yau's 2010 text \cite{Yau10}, but
...
literature (including \cite{Knut73}, \cite{Hazewi08a}, \cite{Hazewi08b}
and \cite{Yau10}) denotes as \textit{pre-}$\lambda
$\textit{-rings} what we call $\lambda$-rings and denotes as $\lambda
$\textit{-rings} what we call special $\lambda$-rings. Even worse, the
notations in \cite{FulLan85} are totally inconsistent\footnote{Often, ``$\lambda$-ring''
in \cite{FulLan85} means ``$\lambda$-ring with a positive structure'' (such $\lambda
$-rings are automatically special), but sometimes it simply means ``$\lambda
$-ring''.}.
...
Fulton/Lang \cite{FulLan85} motivate $\lambda$-rings through vector bundles. Here we are
...
that it is a special $\lambda$-ring is sketched in \cite[Example on page 95]{Seiler88},
...
[The result of Exercise 4.4 is a result by John Hopkinson
(\cite[Proposition 2.1]{Hopkin06}).
His proof is different from the one I give in the solutions. He also
...
sign).\footnote{In \cite{Knut73}, Knutson writes $"+"$, $"-"$ and $"\cdot"$ (with
quotation marks) instead of $\widehat{+},$ $\widehat{-}$ and $\widehat{\cdot}$
for the newly-defined operations. In \cite{FulLan85}, Fulton and Lang simply write $+,$
...
principles}\footnote{We are following \cite[pp. 25--27]{Knut73} here, though our Theorem
8.1 is not exactly what \cite{Knut73} calls ``verification principle''.}. Before we
...
}_{T}$ to distinguish it from the map $\psi_{T}$ in \cite{FulLan85} (which is more or less
...
notations. I am trying to generalize \cite[I \S 6]{FulLan85} (mostly because it is
slightly flawed\footnote{\cite[I \S 6, p. 24]{FulLan85} states that $\operatorname*{td}%
...
graded ring; this notion was defined in \cite{Grin-w4a}. Now let $A$ be the graded ring
$\mathbf{Z}\left[  U_{1},U_{2},...,U_{m}\right]  $, where the grading is given
by the total degree (thus $U_{1}$, $U_{2}$, $...$, $U_{m}$ all lie in the
$1$-st graded component $A_{1}$). According to \cite[Theorem 1 \textbf{(a)}]{Grin-w4a},
the set%
\[
...
denote it might not be standard terminology.\footnote{For instance, what \cite{FulLan85}
...
[Again, \cite{FulLan85} considers only $\theta^{j}:=\theta_{1}^{j},$ which again is
...
Almost all $\lambda$-rings in Fulton/Lang \cite{FulLan85} and many $\lambda$-rings in
...
:K\rightarrow\mathbb{Z}$ be a surjective\footnote{I am quoting this from \cite{FulLan85}.
...
\par
Fulton and Lang do not make this assumption in \cite{FulLan85}, but this is a mistake on
...
element $u\in\mathbf{E}$, the inverse of $u$ must lie in $\mathbf{E}$ as well.
Hence, if we would omit this assumption (as Fulton and Lang do in \cite{FulLan85}), the
...
\textit{splitting principle}, and is what Fulton/Lang \cite{FulLan85} use instead of
