...
%TCIDATA{LastRevised=Wednesday, September 02, 2009 13:51:53}
...
\textbf{5)} Let $\left(  K,\left(  \lambda^{i}\right)  _{i\in\mathbb{N}%
...
$i\in\mathbb{N}$, which in turn means that $\mu_{i}\circ f=f\circ\lambda^{i}$
...
T\right]  \right]  $ of formal power series in the indeterminate $T$ over the
ring $K$. For every $\mu\in K$, we can define a $K$-algebra homomorphism
$\operatorname*{ev}_{\mu T}:K\left[  \left[  T\right]  \right]  \rightarrow
K\left[  \left[  T\right]  \right]  $ by setting $\operatorname*{ev}_{\mu
...
t\right]  \right]  $ before $t^{i}$.

Here, $\lambda_{S^{j}t}\left(  a_{j}\right)  $ is defined as
$\operatorname*{ev}_{S^{j}t}\left(  \lambda_{t}\left(  a_{j}\right)  \right)
$.

Then, $\left(  K\left[  S\right]  ,\left(  \overline{\lambda}^{i}\right)
_{i\in\mathbb{N}}\right)  $ is a $\lambda$-ring. The ring $K$ is a
sub-$\lambda$-ring of $\left(  K\left[  S\right]  ,\left(  \overline{\lambda
}^{i}\right)  _{i\in\mathbb{N}}\right)  $. Besides, $\overline{\lambda}%
^{i}\left(  S\right)  =0$ for every $i>0$.
\end{quote}

\textit{Proof of Theorem 3.3.} The definition of $\overline{\lambda}^{i}$
makes it clear that $\overline{\lambda}_{t}\left(  \sum\limits_{j\in
\mathbb{N}}a_{j}S^{j}\right)  =\prod\limits_{j\in\mathbb{N}}\lambda_{S^{j}%
t}\left(  a_{j}\right)  $ for every $\sum\limits_{j\in\mathbb{N}}a_{j}S^{j}\in
K\left[  S\right]  $. But by Theorem 2.1 \textbf{(a)}, proving that $\left(
K\left[  S\right]  ,\left(  \overline{\lambda}^{i}\right)  _{i\in\mathbb{N}%
}\right)  $ is a $\lambda$-ring boils down to verifying that%
\[
...
_{i\in\mathbb{N}}\right)  $ is a $\lambda$-ring. The rest of Theorem 3.3 is
yet easier to verify.
...
\textit{Exercise 3.3.} \textbf{(a)} Let $K$ be a $\mathbb{Q}$-algebra. Let
$p\in K\left[  \left[  t\right]  \right]  $ be a formal power series with
...
$\exp\left(  x\ln\left(  1+pt\right)  \right)  $) before $t^{i}$. Prove that
...
M\right]  \right)  \left[  \left[  t\right]  \right]  $ before $t^{i}$.

Here, $\lambda_{mt}\left(  \alpha_{m}\right)  $ denotes the image of the
formal power series $\lambda_{t}\left(  \alpha_{m}\right)  $ under the
$K\left[  M\right]  $-algebra homomorphism $\left(  K\left[  M\right]
\right)  \left[  \left[  t\right]  \right]  \rightarrow\left(  K\left[
M\right]  \right)  \left[  \left[  t\right]  \right]  $ which maps $t$ to
$mt$. (In other words, $\lambda_{mt}\left(  \alpha_{m}\right)  $ is what we
get if we replace every $t$ by $mt$ in the power series $\lambda_{t}\left(
\alpha_{m}\right)  .$)
...
$\mathbb{N}$). \footnote{\textit{Hints to solution:} Same idea as for Exercise
3.4.}
\end{quotation}
...
Note that we have denoted the newly-defined addition and multiplication on the
set $1+K\left[  \left[  T\right]  \right]  ^{+}$ by $\widehat{+}$ and
$\widehat{\cdot}$ in order to distinguish them from the addition $+$ and
multiplication $\cdot$ inherited from $K\left[  \left[  T\right]  \right]  $.
We will later continue in this spirit (for instance, we will denote a finite
sum with respect to the addition $\widehat{+}$ by the sign $\widehat{\sum}$,
while a finite sum with respect to the addition $+$ will be written using the
normal $\sum$ sign).
...
the disjoint union $\bigcup\limits_{K^{\prime}\subseteq\operatorname*{Exten}%
K}^{\cdot}\mathcal{P}_{\operatorname*{fin}}^{\ast}\left(  K^{\prime}\right)  $
is defined as $\bigcup\limits_{K^{\prime}\subseteq\operatorname*{Exten}%
...
over $K$. ("Rational function" is a bad misnomer, in this case, but at least
...
\textit{Proof of Theorem 7.3.} Consider the map $\overline{\lambda}%
_{T}:K\left[  S\right]  \rightarrow\Lambda\left(  K\left[  S\right]  \right)
$ defined by $\overline{\lambda}_{T}\left(  \sum\limits_{j\in\mathbb{N}}%
a_{j}S^{j}\right)  =\prod\limits_{j\in\mathbb{N}}\lambda_{S^{j}t}\left(
a_{j}\right)  \in\left(  K\left[  S\right]  \right)  \left[  \left[  t\right]
\right]  $

defined as in Theorem 5.6. Explicitely, this map is given by $\overline
{\lambda}_{T}$

Theorem 5.6 yields that this map $\overline{\lambda}_{T}$ is an additive group
...
First, it is obvious that $\overline{\lambda}_{T}$ maps the multiplicative
unity $1$ of $K\left[  S\right]  $ to the multiplicative unity $1+T$ of
$\Lambda\left(  K\left[  S\right]  \right)  $

Let $E=\left\{  uS^{i}\mid u\in K,\ i\in\mathbb{N}\right\}  $. Obviously, $E$
is a generating set of the $\mathbb{Z}$-module $K\left[  S\right]  $.
...
Here, $\lambda_{S^{j}t}\left(  a_{j}\right)  $ denotes the image of the formal
power series $\lambda_{t}\left(  a_{j}\right)  $ under the $K\left[  S\right]
$-algebra homomorphism $\left(  K\left[  S\right]  \right)  \left[  \left[
t\right]  \right]  \rightarrow\left(  K\left[  S\right]  \right)  \left[
\left[  t\right]  \right]  $ which maps $t$ to $S^{j}t$. (In other words,
$\lambda_{S^{j}t}\left(  a_{j}\right)  $ is what we get if we replace every
$t$ by $S^{j}t$ in the power series $\lambda_{t}\left(  a_{j}\right)  .$)
...
\begin{quote}
Consider the polynomial ring $K\left[  S\right]  $. For every $i\in\mathbb{N}%
$, define a map $\overline{\lambda}^{i}:K\left[  S\right]  \rightarrow
K\left[  S\right]  $ as follows: For every $\sum\limits_{j\in\mathbb{N}}%
a_{j}S^{j}\in K\left[  S\right]  $ (with $a_{j}\in K$ for every $j\in
\mathbb{N}$), let $\overline{\lambda}^{i}\left(  \sum\limits_{j\in\mathbb{N}%
}a_{j}S^{j}\right)  $ be the coefficient of the power series $\prod
\limits_{j\in\mathbb{N}}\lambda_{S^{j}t}\left(  a_{j}\right)  \in\left(
K\left[  S\right]  \right)  \left[  \left[  t\right]  \right]  $ before
$t^{i}$.
