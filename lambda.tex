...
%TCIDATA{LastRevised=Wednesday, September 02, 2009 15:06:19}
...
only if $f$ is a ring homomorphism and satisfies $\mu^{i}\circ f=f\circ
...
\ \ \ \ \ \ \ \ \ \ \left(  \text{since }\widehat{\lambda}^{i}\circ\lambda
_{T}=\lambda_{T}\circ\lambda^{i}\text{, because }\left(  K,\left(  \lambda
^{i}\right)  _{i\in\mathbb{N}}\right)  \text{ is a special }\lambda
\text{-ring}\right) \\
