...
%TCIDATA{LastRevised=Tuesday, April 26, 2016 23:19:21}
...
have $\overline{\lambda}_{T}\left(  \sum\limits_{j\in\mathbb{N}}a_{j}%
S^{j}\right)  =\prod\limits_{j\in\mathbb{N}}\lambda_{S^{j}T}\left(
a_{j}\right)  \in\left(  K\left[  S\right]  \right)  \left[  \left[  T\right]
\right]  $ for every $\sum\limits_{j\in\mathbb{N}}a_{j}S^{j}\in K\left[
S\right]  $ (with $a_{j}\in K$ for every $j\in\mathbb{N}$). Hence, for every
$u\in K\left[  S\right]  $, we have%
\begin{equation}
...
\end{align*}
This proves (\ref{thm.3.3.pf.a.2}).}. Hence, every $u\in K\left[  S\right]  $,
we have $\overline{\lambda}^{0}\left(  u\right)  =1$ (this is obtained by
comparing coefficients before $T^{0}$ in the equality (\ref{thm.3.3.pf.a.2}))
and $\overline{\lambda}^{1}\left(  u\right)  =u$ (this is obtained by
comparing coefficients before $T^{1}$ in the equality (\ref{thm.3.3.pf.a.2})).

Therefore, by Theorem 2.1 \textbf{(a)}, proving that $\left(  K\left[
S\right]  ,\left(  \overline{\lambda}^{i}\right)  _{i\in\mathbb{N}}\right)  $
is a $\lambda$-ring boils down to verifying that%
\[
...
\begin{proof}
[Proof of Theorem 4.2 (\textbf{very} roughly sketched).]\textbf{(a)} Consider
$P$ as a polynomial in the indeterminates $V_{1},$ $V_{2},$ $...,$ $V_{n}$
over the ring $K\left[  U_{1},U_{2},...,U_{m}\right]  $. Then, $P$ is a
symmetric polynomial in these indeterminates $V_{1},$ $V_{2},$ $...,$ $V_{n}$,
so Theorem 4.1 \textbf{(a)} yields the existence of one and only one
...
\begin{proof}
[Proof of Theorem 8.2.]This is clear, since $\left(  \operatorname*{coeff}%
...
\lambda^{i}\left(  x\right)  T^{i}\right)  =\lambda^{1}\left(  x\right)  =x$
...
\begin{proof}
[Proof of Proposition 10.7.]\textit{1st Step:} For every $j\in\mathbb{N}$, we
have%
\begin{align*}
