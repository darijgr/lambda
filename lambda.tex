...
%TCIDATA{LastRevised=Wednesday, September 02, 2009 20:07:17}
...
\[
\wedge^{k}\left(  V\oplus W\right)  \cong\bigoplus_{i=0}^{k}\wedge^{i}\left(
V\right)  \otimes\wedge^{k-i}\left(  W\right)
\]
...
_{i\in\mathbb{N}}\right)  $ be a special $\lambda$-ring. Let $I$ be a set, and
let $J_{i}$ be a set for every $i\in I$.

First, let us introduce a notation: If $P\in\mathbb{Z}\left[  X_{i,j}\mid i\in
I,\text{ }j\in J_{i}\right]  $ is a polynomial in the variables $X_{i,j}$, and
$a_{i,j}$ is an element of a ring $R$ for every $i\in I$ and every $j\in
J_{i}$, then we denote by $P\left(  X_{i,j}\mid a_{i,j}\right)  $ the value of
the polynomial $P$ when each variable $X_{i,j}$ is assigned the value
$a_{i,j}$.
...
\begin{align*}
0 &  =\operatorname*{coeff}\nolimits_{1}\left(  \widehat{P}\left(  X_{i,j}%
\mid\widehat{\lambda}^{j}\left(  u_{i}\right)  \right)  \right)  =P\left(
X_{i,j}\mid\operatorname*{coeff}\nolimits_{1}\left(  \widehat{\lambda}%
^{j}\left(  u_{i}\right)  \right)  \right)  \\
...
\begin{center}
\fbox{\textbf{Augmentation OR Adams operations}}
\end{center}
