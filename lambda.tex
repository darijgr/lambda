...
%TCIDATA{LastRevised=Saturday, October 08, 2011 00:33:39}
...
\textit{Version 0.0.10, last revised 29 Sep 2011}
\end{center}
...
$\mathbb{Z}$ of integers; however, in some cases it will not (for example,
even if we would formulate Theorem 10.19 for $\mathbf{Z}=\mathbb{Z}$ only, the
proof of this theorem would require considering other rings $\mathbf{Z}$ !).
\end{itemize}
...
,X_{2},...,X_{j}\right)  \\\text{(by th 2nd step)}}}T^{j}\\
...
_{j}\right]  $) for every $j\in\mathbb{N}$.
\end{quote}
...
Now we forget that we fixed $m$. Instead, fix some $j\in\mathbb{N}$, and take
$m=j$. Then, we have just proved that
\[
X_{j}u^{j}=\operatorname*{Td}\nolimits_{1+ut,j}\left(  X_{1},X_{2}%
...
\begin{align*}
\operatorname*{td}\nolimits_{1+ut,T}\left(  x\right)   &  =\sum_{j\in
\mathbb{N}}\underbrace{\operatorname*{Td}\nolimits_{1+ut,j}}_{\substack{=u^{j}%
\alpha^{j}\\\text{(by Proposition 10.4)}}}\left(  \lambda^{1}\left(  x\right)
,\lambda^{2}\left(  x\right)  ,...,\lambda^{j}\left(  x\right)  \right)
T^{j}\\
&  =\sum_{j\in\mathbb{N}}\underbrace{\left(  u^{j}\alpha^{j}\right)  \left(
\lambda^{1}\left(  x\right)  ,\lambda^{2}\left(  x\right)  ,...,\lambda
^{j}\left(  x\right)  \right)  }_{=u^{j}\lambda^{j}\left(  x\right)  }%
T^{j}=\sum_{j\in\mathbb{N}}\underbrace{u^{j}\lambda^{j}\left(  x\right)
T^{j}}_{=\lambda^{j}\left(  x\right)  u^{j}T^{j}}=\sum_{j\in\mathbb{N}}%
\lambda^{j}\left(  x\right)  u^{j}T^{j}.
\end{align*}
Compared with%
\[
...
T^{j}\right)  =\sum\limits_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)
u^{j}T^{j}%
\]
(by the definition of $\operatorname*{ev}\nolimits_{uT}$), this yields that
$\operatorname*{td}\nolimits_{1+ut,T}\left(  x\right)  =\lambda_{uT}\left(
x\right)  $. This proves Proposition 10.3.

The following proposition gives the first two coefficients of the power series
...
\[
\prod\limits_{i=1}^{m}\varphi\left(  U_{i}T\right)  =\sum_{j\in\mathbb{N}%
}\operatorname*{Td}\nolimits_{\varphi,0}\left(  X_{1},X_{2},...,X_{j}\right)
T^{j}.
\]
...
The Proposition 10.9 just proved allows us to show some nontrivial properties
of $\operatorname*{td}\nolimits_{\varphi,T}$ for $\varphi$ being an arbitrary
power series (rather than just a power series of the form $1+ut$ as in
Proposition 10.3). However, it does not yield any of these properties
directly; it can be directly applied to compute $\operatorname*{td}%
\nolimits_{\varphi,T}\left(  x\right)  $ only if $\varphi$ is a product of
finitely many power series $\varphi_{i}$ for which we already have computed
$\operatorname*{td}\nolimits_{\varphi_{i},T}$ (and the only such series
$\varphi_{i}$, at the moment, are those of the form $1+ut$). To obtain any
results for generic $\varphi$, we need two tricks: First, we need a kind of
continuity (similar to the one we used in Section 5) to reduce the case of
$\varphi$ a power series to the case of $\varphi$ a polynomial. Second, we
need to split every arbitrary polynomial $\varphi$ with constant term equal to
$1$ into a product of polynomials of the form $1+ut$; this will be done by an
appropriate extension of the ring $\mathbf{Z}$ (again, similarly to how we
extended $K$ in Section 5). First, let us introduce the version of continuity
that we need.

The following two definitions are copies of two definitions which we made in
Section 5, with the only difference that the ring that used to be called $K$
in Section 5 is called $\mathbf{Z}$ here, and that the variable that used to
be $T$ in Section 5 is $t$ here.

\begin{quote}
\textbf{Definition.} Let $\mathbf{Z}$ be a ring. Let $\mathbf{Z}\left[
t\right]  ^{+}$ be the subset of $\mathbf{Z}\left[  t\right]  $ defined by%
\begin{align*}
\mathbf{Z}\left[  t\right]  ^{+}  &  =t\mathbf{Z}\left[  t\right]  =\left\{
\sum_{i\in\mathbb{N}}a_{i}t^{i}\in\mathbf{Z}\left[  t\right]  \ \mid\ a_{i}%
\in\mathbf{Z}\text{ for all }i,\text{ and }a_{0}=0\right\} \\
&  =\left\{  p\in\mathbf{Z}\left[  t\right]  \ \mid\ p\text{ is a polynomial
with constant term }0\right\}  .
\end{align*}
Then, the set $1+\mathbf{Z}\left[  t\right]  ^{+}$ is a subset of
$1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}.$ The elements of
$1+\mathbf{Z}\left[  t\right]  ^{+}$ are polynomials.

\textbf{Definition.} Let $\mathbf{Z}$ be a ring. As a $\mathbf{Z}$-module,
$\mathbf{Z}\left[  \left[  t\right]  \right]  =\bigoplus\limits_{k\in
\mathbb{N}}\mathbf{Z}t^{k}$. Now, we define the so-called $\left(
\mathbf{t}\right)  $\textit{-topology} on the ring $\mathbf{Z}\left[  \left[
t\right]  \right]  $ as the topology generated by%
\[
\left\{  0,\varnothing\right\}  \cup\left\{  u+\sum_{k\geq N}\mathbf{Z}%
t^{k}\ \mid\ u\in\mathbf{Z}\left[  \left[  t\right]  \right]  \text{ and }%
N\in\mathbb{N}\right\}  .
\]
In other words, the open sets of this topology should be $0$, $\varnothing$
and all translates of the submodules $\sum\limits_{k\geq N}\mathbf{Z}t^{k}$
for $N\in\mathbb{N}$, as well as the unions of these sets. (Note that the sum
$\sum\limits_{k\geq N}\mathbf{Z}t^{k}$ is actually a direct sum. Also note
that every translate of the submodule $\sum\limits_{k\geq N}\mathbf{Z}t^{k}$
for $N\in\mathbb{N}$ actually has the form $p+\sum\limits_{k\geq N}%
\mathbf{Z}t^{k}$ for a polynomial $p\in\mathbf{Z}\left[  t\right]  $ of degree
$<N$, and this polynomial is uniquely determined.)
\end{quote}

Now, we have:

\begin{quote}
\textbf{Theorem 10.10.} Let $\mathbf{Z}$ be a ring. The $\left(  t\right)
$-topology on the ring $\mathbf{Z}\left[  \left[  t\right]  \right]  $
restricts to a topology on its subset $1+\mathbf{Z}\left[  \left[  t\right]
\right]  ^{+}$; we call this topology the $\left(  t\right)  $%
\textit{-topology} again. Whenever we say "open", "continuous", "dense", etc.,
we are referring to this topology.

\textbf{(a)} The subset $1+\mathbf{Z}\left[  t\right]  ^{+}$ is dense in
$1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}$.

\textbf{(b)} Let $K$ be a ring. Let $f:1+\mathbf{Z}\left[  \left[  t\right]
\right]  ^{+}\rightarrow K\left[  \left[  T\right]  \right]  $ be a map such
that for every $n\in\mathbb{N}$ there exists some $N\in\mathbb{N}$ such that
...
remaining ones). Then, $f$ is continuous. (Here, the topology on $K\left[
\left[  T\right]  \right]  $ is supposed to be the $\left(  T\right)  $-adic
topology defined in Section 5.)

\textbf{(c)} The topological spaces $\mathbf{Z}\left[  \left[  t\right]
\right]  $ and $1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}$ are
Hausdorff spaces.
\end{quote}

\textit{Proof of Theorem 10.10.} The parts \textbf{(a)} and \textbf{(c)} of
Theorem 10.10 are obviously obtained from the parts \textbf{(a)} and
\textbf{(e)} of Theorem 5.5 by renaming the ring $K$ as $\mathbf{Z}$ and
renaming the variable $T$ as $t$. Hence, they follow from Theorem 5.5. Part
\textbf{(b)} of Theorem 10.10 is also true (it is an exercise in topology,
proven in the same way as Theorem 5.5 \textbf{(b)}). This proves Theorem 10.10.

The good thing about the topology on $1+\mathbf{Z}\left[  \left[  t\right]
\right]  ^{+}$ just defined is that it makes the map $1+\mathbf{Z}\left[
\left[  t\right]  \right]  ^{+}\rightarrow K\left[  \left[  T\right]  \right]
$, $\varphi\mapsto\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)  $
continuous for every given $x\in K$:

\begin{quote}
\textbf{Proposition 10.11.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $x\in K$. Then, the map%
\[
1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}\rightarrow K\left[
\left[  T\right]  \right]  ,\ \ \ \ \ \ \ \ \ \ \varphi\mapsto
\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)
\]
is continuous. Here, the topology on $1+\mathbf{Z}\left[  \left[  t\right]
\right]  ^{+}$ is supposed to be the $\left(  t\right)  $-topology, and the
topology on $K\left[  \left[  T\right]  \right]  $ is supposed to be the
$\left(  T\right)  $-topology defined in Section 5.
\end{quote}

\textit{Proof of Proposition 10.11.} Let $f$ denote the map
\[
1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}\rightarrow K\left[
\left[  T\right]  \right]  ,\ \ \ \ \ \ \ \ \ \ \varphi\mapsto
\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)  .
\]
Then, in order to verify Proposition 10.11, we must prove that this map $f$ is continuous.

\textit{1st Step:} Let $n\in\mathbb{N}$. Let $\varphi\in1+\mathbf{Z}\left[
\left[  t\right]  \right]  ^{+}$ and $\psi\in1+\mathbf{Z}\left[  \left[
t\right]  \right]  ^{+}$ be two power series such that the first $n$
coefficients\footnote{Note that when we say "the first $n$ coefficients" (of
some power series), we mean the coefficients before $t^{0}$, $t^{1}$, $...$,
$t^{n-1}$.} of $\varphi$ are equal to the respective coefficients of $\psi$.
Then, the first $n$ coefficients of the power series $\operatorname*{td}%
\nolimits_{\varphi,T}\left(  x\right)  $ are equal to the respective
coefficients of the power series $\operatorname*{td}\nolimits_{\psi,T}\left(
x\right)  $.
...
\equiv\psi\operatorname{mod}t^{n}$ in the ring $\mathbf{Z}\left[  \left[
t\right]  \right]  $. Thus, there exists some formal power series $\eta
\in\mathbf{Z}\left[  \left[  t\right]  \right]  $ such that $\varphi-\psi=\eta
t^{n}$. Consider such an $\eta$.

Consider the polynomial ring $\mathbf{Z}\left[  U_{1},U_{2},...,U_{m}\right]
$ and its elements $X_{i}=\sum\limits_{\substack{S\subseteq\left\{
1,2,...,m\right\}  ;\\\left\vert S\right\vert =i}}\prod\limits_{k\in S}U_{k}$
as in the definition of $\operatorname*{Td}\nolimits_{\varphi,j}$.
...
\text{power series }\eta t^{n}\in\mathbf{Z}\left[  \left[  t\right]  \right]
\text{ to }U_{i}T\text{, and not a product of }\eta t^{n}\\
...
\operatorname{mod}T^{n}$ in the ring $\left(  \mathbf{Z}\left[  U_{1}%
,U_{2},...,U_{m}\right]  \right)  \left[  \left[  T\right]  \right]  $.
Multiplying the congruences $\varphi\left(  U_{i}T\right)  \equiv\psi\left(
...
In fact, let $\mathfrak{Q}_{1}\in\mathbf{Z}\left[  \alpha_{1},\alpha
_{2},...,\alpha_{m}\right]  $ be the polynomial defined by $\mathfrak{Q}%
_{1}=\operatorname*{Td}\nolimits_{\varphi,m}$. Let $\mathfrak{Q}_{2}%
\in\mathbf{Z}\left[  \alpha_{1},\alpha_{2},...,\alpha_{m}\right]  $ be the
polynomial defined by $\mathfrak{Q}_{2}=\operatorname*{Td}\nolimits_{\psi,m}$.
We are now going to prove that $\mathfrak{Q}_{1}=\mathfrak{Q}_{2}$.

Applying Theorem 4.1 \textbf{(a)} to $\mathbf{Z}$ and $\operatorname*{Td}%
\nolimits_{\varphi,m}\left(  X_{1},X_{2},...,X_{m}\right)  $ instead of $K$
and $P$, we conclude that there exists one and only one polynomial
$Q\in\mathbf{Z}\left[  \alpha_{1},\alpha_{2},...,\alpha_{m}\right]  $ such
that $\operatorname*{Td}\nolimits_{\varphi,m}\left(  X_{1},X_{2}%
,...,X_{m}\right)  =Q\left(  X_{1},X_{2},...,X_{m}\right)  $. In particular,
there exists \textit{at most one} such polynomial $Q\in\mathbf{Z}\left[
\alpha_{1},\alpha_{2},...,\alpha_{m}\right]  $. Hence,
\begin{equation}
\left(
\begin{array}
[c]{c}%
\text{if }\mathfrak{Q}_{1}\in\mathbf{Z}\left[  \alpha_{1},\alpha
_{2},...,\alpha_{m}\right]  \text{ and }\mathfrak{Q}_{2}\in\mathbf{Z}\left[
...
Now,%
\begin{align*}
\operatorname*{Coeff}\nolimits_{m}\left(  \operatorname*{td}\nolimits_{\varphi
,T}\left(  x\right)  \right)   &  =\operatorname*{Coeff}\nolimits_{m}\left(
\sum\limits_{j\in\mathbb{N}}\operatorname*{Td}\nolimits_{\varphi,j}\left(
\lambda^{1}\left(  x\right)  ,\lambda^{2}\left(  x\right)  ,...,\lambda
^{j}\left(  x\right)  \right)  T^{j}\right)  \ \ \ \ \ \ \ \ \ \ \left(
\text{by (\ref{ToddDef})}\right) \\
&  =\operatorname*{Td}\nolimits_{\varphi,m}\left(  \lambda^{1}\left(
x\right)  ,\lambda^{2}\left(  x\right)  ,...,\lambda^{m}\left(  x\right)
\right)  .
\end{align*}
The same argument, applied to $\psi$ instead of $\varphi$, yields%
\[
\operatorname*{Coeff}\nolimits_{m}\left(  \operatorname*{td}\nolimits_{\psi
,T}\left(  x\right)  \right)  =\operatorname*{Td}\nolimits_{\psi,m}\left(
\lambda^{1}\left(  x\right)  ,\lambda^{2}\left(  x\right)  ,...,\lambda
^{m}\left(  x\right)  \right)  .
\]
Thus,%
\begin{align*}
\operatorname*{Coeff}\nolimits_{m}\left(  \operatorname*{td}\nolimits_{\varphi
,T}\left(  x\right)  \right)   &  =\underbrace{\operatorname*{Td}%
\nolimits_{\varphi,m}}_{=\operatorname*{Td}\nolimits_{\psi,m}}\left(
\lambda^{1}\left(  x\right)  ,\lambda^{2}\left(  x\right)  ,...,\lambda
^{m}\left(  x\right)  \right) \\
&  =\operatorname*{Td}\nolimits_{\psi,m}\left(  \lambda^{1}\left(  x\right)
,\lambda^{2}\left(  x\right)  ,...,\lambda^{m}\left(  x\right)  \right)
=\operatorname*{Coeff}\nolimits_{m}\left(  \operatorname*{td}\nolimits_{\psi
...
\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)  \right)
=\operatorname*{Coeff}\nolimits_{m}\left(  \operatorname*{td}\nolimits_{\psi
,T}\left(  x\right)  \right)  $ for every $m\in\left\{  0,1,...,n-1\right\}
$. In other words, for every $m\in\left\{  0,1,...,n-1\right\}  $, the $m$-th
coefficient of the power series $\operatorname*{td}\nolimits_{\varphi
,T}\left(  x\right)  $ equals the respective coefficient of the power series
$\operatorname*{td}\nolimits_{\psi,T}\left(  x\right)  $. In other words, the
first $n$ coefficients of the power series $\operatorname*{td}%
\nolimits_{\varphi,T}\left(  x\right)  $ are equal to the respective
coefficients of the power series $\operatorname*{td}\nolimits_{\psi,T}\left(
x\right)  $.
...
\textit{2nd Step:} Let $n\in\mathbb{N}$. Let $\varphi\in1+\mathbf{Z}\left[
\left[  t\right]  \right]  ^{+}$ and $\psi\in1+\mathbf{Z}\left[  \left[
t\right]  \right]  ^{+}$ be two power series such that the first $n$
coefficients\footnote{Note that when we say "the first $n$ coefficients" (of
some power series), we mean the coefficients before $t^{0}$, $t^{1}$, $...$,
$t^{n-1}$.} of $\varphi$ are equal to the respective coefficients of $\psi$.
Then, the first $n$ coefficients of the power series $f\left(  \varphi\right)
$ are equal to the respective coefficients of the power series $f\left(
\psi\right)  $.

\textit{Proof.} This is just an equivalent restatement of the 1st Step, since
$f\left(  \varphi\right)  =\operatorname*{td}\nolimits_{\varphi,T}\left(
x\right)  $ (by the definition of $f$) and $f\left(  \psi\right)
=\operatorname*{td}\nolimits_{\psi,T}\left(  x\right)  $ (by the definition of
$f$).
...
$1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}$ have the same first $n$
...
10.10 \textbf{(b)}, this yields that $f$ is continuous.
...
\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)
\]
is continuous. Proposition 10.11 is thus proven.

So much for the topology on $1+\mathbf{Z}\left[  \left[  t\right]  \right]
^{+}$. We now discuss extensions of $\mathbf{Z}$ that make polynomials factor.

By renaming the ring $K$ as $\mathbf{Z}$ and the variable $T$ as $t$ in
Theorem 5.2, we obtain the following fact:

\begin{quote}
\textbf{Theorem 10.12.} Let $\mathbf{Z}$ be a ring. For every element
$p\in1+\mathbf{Z}\left[  t\right]  ^{+}$, there exists an integer $n$ (the
degree of the polynomial $p$), a finite-free extension ring $\mathbf{Z}_{p}$
of the ring $\mathbf{Z}$ and $n$ elements $p_{1},$ $p_{2},$ $...,$ $p_{n}$ of
this extension ring $\mathbf{Z}_{p}$ such that $p=\prod\limits_{i=1}%
^{n}\left(  1+p_{i}t\right)  $ in $\mathbf{Z}_{p}\left[  t\right]  $.
\end{quote}

This is not yet all. We need a slight generalization of our notion of
$\varphi$-Todd homomorphisms to the case when the power series $\varphi$ does
not lie in $1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}$ but, instead,
lies in $1+\mathbf{Z}^{\prime}\left[  \left[  t\right]  \right]  ^{+}$ for
$\mathbf{Z}^{\prime}$ being a commutative $\mathbf{Z}$-algebra. In this case,
it turns out, not much will change - but, of course, $\operatorname*{td}%
\nolimits_{\varphi,T}$ will no longer be a map $K\rightarrow K\left[  \left[
T\right]  \right]  $ but instead will be a map $K\rightarrow\left(
K\otimes_{\mathbf{Z}}\mathbf{Z}^{\prime}\right)  \left[  \left[  T\right]
\right]  $. Here is the precise definition:

\begin{quote}
\textbf{Definition.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ be a power series with constant term equal to $1$. We
define a map $\operatorname*{td}\nolimits_{\varphi,T,\mathbf{Z}^{\prime}%
...
1$'s. Here is the analogue of Proposition 10.3:

\begin{quote}
\textbf{Proposition 10.14.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $u\in\mathbf{Z}^{\prime}$. For every $x\in K$, we
have $\operatorname*{td}\nolimits_{1+ut,T,\mathbf{Z}^{\prime}}\left(
x\right)  =\lambda_{\left(  1\otimes u\right)  T}\left(  x\right)  $, where
$\lambda_{\left(  1\otimes u\right)  T}\left(  x\right)  $ means
$\operatorname*{ev}\nolimits_{\left(  1\otimes u\right)  T}\left(  \lambda
_{T}\left(  x\right)  \right)  $.
\end{quote}

\textit{Proof of Proposition 10.14.} The following proof is, of course, a very
direct calque of our above proof of Proposition 10.3.

Let $x\in K$. Applying (\ref{ToddDefZ'}) to $\varphi=1+ut$, we obtain%
\begin{align*}
\operatorname*{td}\nolimits_{1+ut,T,\mathbf{Z}^{\prime}}\left(  x\right)   &
=\sum_{j\in\mathbb{N}}\underbrace{\operatorname*{Td}\nolimits_{1+ut,j}%
}_{\substack{=u^{j}\alpha^{j}\\\text{(by Proposition 10.4,}\\\text{applied to
}\mathbf{Z}^{\prime}\text{ instead of }\mathbf{Z}\text{)}}}\left(  \lambda
^{1}\left(  x\right)  \otimes1,\lambda^{2}\left(  x\right)  \otimes
1,...,\lambda^{j}\left(  x\right)  \otimes1\right)  T^{j}\\
&  =\sum_{j\in\mathbb{N}}\underbrace{\left(  u^{j}\alpha^{j}\right)  \left(
\lambda^{1}\left(  x\right)  \otimes1,\lambda^{2}\left(  x\right)
\otimes1,...,\lambda^{j}\left(  x\right)  \otimes1\right)  }_{\substack{=u^{j}%
\left(  \lambda^{j}\left(  x\right)  \otimes1\right)  =\lambda^{j}\left(
x\right)  \otimes u^{j}\\=\lambda^{j}\left(  x\right)  \cdot\left(  1\otimes
u^{j}\right)  }}T^{j}\\
&  =\sum_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)  \cdot\underbrace{\left(
1\otimes u^{j}\right)  }_{=\left(  1\otimes u\right)  ^{j}}T^{j}=\sum
_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)  \left(  1\otimes u\right)
^{j}T^{j}.
\end{align*}
Compared with%
\[
\lambda_{\left(  1\otimes u\right)  T}\left(  x\right)  =\operatorname*{ev}%
\nolimits_{\left(  1\otimes u\right)  T}\left(  \underbrace{\lambda_{T}\left(
x\right)  }_{=\sum\limits_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)  T^{j}%
}\right)  =\operatorname*{ev}\nolimits_{\left(  1\otimes u\right)  T}\left(
\sum\limits_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)  T^{j}\right)
=\sum_{j\in\mathbb{N}}\lambda^{j}\left(  x\right)  \left(  1\otimes u\right)
^{j}T^{j}%
\]
(by the definition of $\operatorname*{ev}\nolimits_{\left(  1\otimes u\right)
T}$), this yields that $\operatorname*{td}\nolimits_{1+ut,T,\mathbf{Z}%
^{\prime}}\left(  x\right)  =\lambda_{\left(  1\otimes u\right)  T}\left(
x\right)  $. This proves Proposition 10.14.

Similarly, here is the analogue of Proposition 10.5:

\begin{quote}
\textbf{Proposition 10.15.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ be a power series with constant term equal to $1$.
...
\textit{Proof of Proposition 10.15.} Let $x\in K$.
...
Proposition 10.15 is now proven.

The analogue of Proposition 10.7:

\begin{quote}
\textbf{Proposition 10.16.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ and $\psi\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ be two power series with constant terms equal to $1$.
For every $x\in K$, we have $\operatorname*{td}\nolimits_{\varphi
\psi,T,\mathbf{Z}^{\prime}}\left(  x\right)  =\operatorname*{td}%
\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(  x\right)  \operatorname*{td}%
\nolimits_{\psi,T,\mathbf{Z}^{\prime}}\left(  x\right)  $.
\end{quote}

\textit{Proof of Proposition 10.16.} To obtain a proof of Proposition 10.16,
...
\item Replace all references to Proposition 10.7 by references to Proposition 10.16.
\end{itemize}

Next, the analogue of Proposition 10.9:

\begin{quote}
\textbf{Proposition 10.17.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $m\in\mathbb{N}$. For every $i\in\left\{
1,2,...,m\right\}  $, let $\varphi_{i}\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ be a power series with constant term equal to $1$.
...
\textit{Proof of Proposition 10.17.} This can be proven by induction over $m$.
The induction base (the case $m=0$) requires showing that $\operatorname*{td}%
\nolimits_{1,T,\mathbf{Z}^{\prime}}\left(  x\right)  =1$, but this follows
from Proposition 10.14\footnote{In fact, Proposition 10.14 (applied to $u=0$)
...
Proposition 10.16. Thus Proposition 10.17 is proven.

Finally, the analogue of Proposition 10.11:

\begin{quote}
\textbf{Proposition 10.18.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $x\in K$. Then, the map%
\[
1+\mathbf{Z}^{\prime}\left[  \left[  t\right]  \right]  ^{+}\rightarrow\left(
K\otimes_{\mathbf{Z}}\mathbf{Z}^{\prime}\right)  \left[  \left[  T\right]
\right]  ,\ \ \ \ \ \ \ \ \ \ \varphi\mapsto\operatorname*{td}%
\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(  x\right)
\]
is continuous. Here, the topology on $1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ is supposed to be the $\left(  t\right)  $-topology
(but for $\mathbf{Z}$ replaced by $\mathbf{Z}^{\prime}$), and the topology on
$\left(  K\otimes_{\mathbf{Z}}\mathbf{Z}^{\prime}\right)  \left[  \left[
T\right]  \right]  $ is supposed to be the $\left(  T\right)  $-topology
defined in Section 5 (but for $K$ replaced by $K\otimes_{\mathbf{Z}}%
\mathbf{Z}^{\prime}$).
\end{quote}

\textit{Proof of Proposition 10.18.} To obtain a proof of Proposition 10.18,
read the proof of Proposition 10.11, doing the following replacements:

\begin{itemize}
\item Replace every $\mathbf{Z}$ by $\mathbf{Z}^{\prime}$.

\item Replace every $K\left[  \left[  T\right]  \right]  $ by $\left(
K\otimes_{\mathbf{Z}}\mathbf{Z}^{\prime}\right)  \left[  \left[  T\right]
\right]  $.

\item Replace "Proposition 10.1" by "Proposition 10.1 (applied to
$\mathbf{Z}^{\prime}$ instead of $\mathbf{Z}$)".
...
Now we can reap the harvest of these preparations:

\begin{quote}
\textbf{Theorem 10.19.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}\left[  \left[
t\right]  \right]  ^{+}$ be a power series with constant term equal to $1$.
...
Even if we were interested only in this theorem, the only way to prove it
known to me is by generalizing it:

\begin{quote}
\textbf{Theorem 10.20.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\mathbf{Z}^{\prime}$ be a commutative
$\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}^{\prime}\left[  \left[
t\right]  \right]  ^{+}$ be a power series with constant term equal to $1$.
Let $x\in K$ and $y\in K$. Then, $\operatorname*{td}\nolimits_{\varphi
,T,\mathbf{Z}^{\prime}}\left(  x\right)  \cdot\operatorname*{td}%
\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(  y\right)  =\operatorname*{td}%
\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(  x+y\right)  $.
\end{quote}

\textit{Proof of Theorem 10.20.} \textit{1st Step:} We have
$\operatorname*{td}\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(  x\right)
\cdot\operatorname*{td}\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(
y\right)  =\operatorname*{td}\nolimits_{\varphi,T,\mathbf{Z}^{\prime}}\left(
x+y\right)  $ whenever $\varphi\in1+\mathbf{Z}^{\prime}\left[  t\right]  ^{+}$.

\textit{Proof.} Let $\varphi\in1+\mathbf{Z}^{\prime}\left[  t\right]  ^{+}$.
According to Theorem 10.12 (applied to $\varphi$ and $\mathbf{Z}^{\prime}$
instead of $p$ and $\mathbf{Z}$), there exists an integer $n$ (the degree of
the polynomial $\varphi$), a finite-free extension ring $\mathbf{Z}_{\varphi
}^{\prime}$ of the ring $\mathbf{Z}^{\prime}$ and $n$ elements $p_{1},$
$p_{2},$ $...,$ $p_{n}$ of this extension ring $\mathbf{Z}_{\varphi}^{\prime}$
such that $\varphi=\prod\limits_{i=1}^{n}\left(  1+p_{i}t\right)  $ in
$\mathbf{Z}_{\varphi}^{\prime}\left[  t\right]  $. Consider this ring
$\mathbf{Z}_{\varphi}^{\prime}$ and these $n$ elements $p_{1},$ $p_{2},$
$...,$ $p_{n}$.

The canonical embedding $\mathbf{Z}^{\prime}\rightarrow\mathbf{Z}_{\varphi
}^{\prime}$ is injective. Hence, the map $K\otimes_{\mathbf{Z}}\mathbf{Z}%
^{\prime}\rightarrow K\otimes_{\mathbf{Z}}\mathbf{Z}_{\varphi}^{\prime}$ that
it induces must also be injective [...]

[...]

\textit{Proof of Theorem 10.19.} [...]

[...]

As a consequence of Proposition 10.9, we have:

\begin{quote}
\textbf{Theorem 10.UX.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $\varphi\in1+\mathbf{Z}\left[  t\right]
^{+}$ be a polynomial with constant term equal to $1$. Let us use the
notations of Section 5. Let $\left(  \widetilde{\mathbf{Z}},\left[
u_{1},u_{2},...,u_{n}\right]  \right)  \in\mathbf{Z}^{\operatorname*{int}}$ be
such that $\varphi\left(  T\right)  =\Pi\left(  \widetilde{\mathbf{Z}},\left[
u_{1},u_{2},...,u_{n}\right]  \right)  $. Then,%
\[
\operatorname*{td}\nolimits_{\varphi,T}\left(  x\right)  =\prod\limits_{i=1}%
^{m}\operatorname*{td}\nolimits_{\varphi_{i},T}\left(  x\right)
\ \ \ \ \ \ \ \ \ \ \text{for every }x\in\widetilde{K}.
\]
...
\begin{quote}
\textbf{Definition \& Theorem 10.X1.} Let $\mathbf{Z}$ be a ring. Let
$\varphi\in1+\mathbf{Z}\left[  \left[  t\right]  \right]  ^{+}$ be a power
series with constant term equal to $1$. There exists one and only one
...
\textbf{Theorem 10.X2.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Let $u_{1},$ $u_{2},$ $...,$ $u_{m}$ be
$1$-dimensional elements of $K$. Let $j\in\mathbb{N}$. Then,%
\[
...
\textbf{Theorem 10.X3.} Let $\mathbf{Z}$ be a ring. Let $\left(  K,\left(
\lambda^{i}\right)  _{i\in\mathbb{N}}\right)  $ be a $\lambda$-ring such that
$K$ is a $\mathbf{Z}$-algebra. Then, $\operatorname*{td}_{\varphi,T}\left(
K\right)  \subseteq\Lambda\left(  K\right)  $, and $\operatorname*{td}%
...
\begin{quote}
\textbf{Theorem 10.X4.} Let $\left(  K,\left(  \lambda^{i}\right)
_{i\in\mathbb{N}}\right)  $ be a special $\lambda$-ring. Let $x\in K$. Let
$j\in\mathbb{N}\diagdown\left\{  0\right\}  $. Then, $\left(  \psi^{j}\left[
\left[  T\right]  \right]  \right)  \left(  \lambda_{-T}\left(  x\right)
\right)  =\lambda_{-T}\left(  x\right)  \theta_{T}^{j}\left(  x\right)  $
(where $\psi^{j}\left[  \left[  T\right]  \right]  $ means the homomorphism
$K\left[  \left[  T\right]  \right]  \rightarrow K\left[  \left[  T\right]
\right]  $ defined by $\left(  \psi^{j}\left[  \left[  T\right]  \right]
\right)  \left(  \sum\limits_{i\in\mathbb{N}}a_{i}T^{i}\right)  =\sum
\limits_{i\in\mathbb{N}}\psi^{j}\left(  a_{i}\right)  T^{i}$ for every power
series $\sum\limits_{i\in\mathbb{N}}a_{i}T^{i}\in K\left[  \left[  T\right]
\right]  $).
\end{quote}
...
K\right)  $ (where $\widehat{-}$ means subtraction with respect to the addtion
$\widehat{+}$ in $\Lambda\left(  K\right)  $). The subset $1+K\left[
